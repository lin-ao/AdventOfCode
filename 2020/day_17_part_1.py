...
def load_input(file_path: str) -> list[tuple[int, int, int]]:
    with open(file_path, "r") as file:
        pocket = [(x, y, 0) for x, row in enumerate(file) for y, col in enumerate(row) if col == "#"]
        return pocket


def neighbors(point: tuple[int, int, int]) -> list[tuple[Any]]:
    x = [point[0] - 1, point[0], point[0] + 1]
    y = [point[1] - 1, point[1], point[1] + 1]
    z = [point[2] - 1, point[2], point[2] + 1]
    _neighbors = list(product(x, y, z))
    _neighbors.remove(point)
    return _neighbors


def update(pocket_dimension: list[tuple[int, int, int]], cycle: int) -> int:
    if cycle > 0:
